...
echo "🚀 Deploying TaskFlow Production Environment"
...
kubectl wait --for=condition=available deployment/taskflow-postgres-prod -n taskflow-prod --timeout=300s

# Deploy API
echo "🌐 Deploying API..."
kubectl apply -f api.yaml
kubectl wait --for=condition=available deployment/taskflow-api-prod -n taskflow-prod --timeout=300s

# Deploy Frontend
echo "🎨 Deploying Frontend..."
kubectl apply -f frontend.yaml
kubectl wait --for=condition=available deployment/taskflow-frontend-prod -n taskflow-prod --timeout=300s
...
for i in {1..30}; do
    INGRESS_IP=$(kubectl get ingress taskflow-ingress-prod -n taskflow-prod -o jsonpath='{.status.loadBalancer.ingress[0].ip}' 2>/dev/null)
...
echo ""
echo "🎉 TaskFlow Production deployment completed!"
echo ""
echo "🌐 Access URLs:"
...
echo "  kubectl get pods -n taskflow-prod"
echo "  kubectl logs -f deployment/taskflow-api-prod -n taskflow-prod"
echo "  kubectl logs -f deployment/taskflow-frontend-prod -n taskflow-prod"
echo "  kubectl describe ingress taskflow-ingress-prod -n taskflow-prod"